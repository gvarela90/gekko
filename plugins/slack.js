...
Slack.prototype.processAdvice = function(advice) {
  ...
  const color =
    advice.recommendation === 'long'
      ? 'good'
      : advice.recommendation === 'short'
      ? 'danger'
      : 'warning';
  const body = this.createResponse(
    color,
    'There is a new trend! The advice is to go `' +
...
Slack.prototype.createResponse = function(color, message) {
  ...
  if (config.tradingAdvisor.enabled) {
    prefix = `*[${config.tradingAdvisor.method} | ${config.type}]* `;
  }
  const template = {
...
Slack.prototype.createUserName = function() {
  return (
    config.watch.exchange[0].toUpperCase() +
    config.watch.exchange.slice(1) +
    ' - ' +
...
Slack.prototype.createIconUrl = function() {
  const asset =
    config.watch.asset === 'XBT' ? 'btc' : config.watch.asset.toLowerCase();
  return (
    'https://github.com/cjdowner/cryptocurrency-icons/raw/master/128/icon/' +
    asset +
    '.png'
  );
};
